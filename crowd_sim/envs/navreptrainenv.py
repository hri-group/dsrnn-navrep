...
from a.crowd_sim.envs.crowd_sim import CrowdSim2  # reference to env code  # noqa
...
class NavRepTrainEnv(CrowdSim2):
    ...
    def __init__(self, scenario='train', silent=False, legacy_mode=False, adaptive=False, lidar_legs=True,
        ...
        d['robot_node'] = spaces.Box(low=-np.inf, high=np.inf, shape=(1,7,), dtype = np.float32)
        ...
        d['spatial_edges'] = spaces.Box(low=-np.inf, high=np.inf, shape=(5, 2), dtype=np.float32)
        d['lidar'] = spaces.Box(low=-np.inf, high=np.inf, shape=(1080,), dtype=np.float32)
        ...
        # self.human_num = 0 #change to actual later

    def _convert_obs(self, reset):
        #self.soadrl_sim.human_num = len(self.soadrl_sim.humans)
        ...
        #CHANGE THIS LATER PLEASE OSIF:RSOIILDGUHDLUDH
        obs={}
        # robot node: px, py, r, gx, gy, v_pref, theta
        obs['robot_node'] = [robot.px, robot.py, robot.radius, robot.gx, robot.gy, robot.v_pref, robot.theta]
        visible_humans, num_visibles, human_visibility = self.get_num_human_in_fov(self.soadrl_sim.human_num,
            robot, self.soadrl_sim.humans, self.soadrl_sim.robot_fov, self.soadrl_sim.human_fov)
        # only consider the robot temporal edge and spatial edges pointing from robot to each human
        obs['temporal_edges'] = np.array([robot.vx,robot.vy])
        obs['spatial_edges'] = np.zeros((self.soadrl_sim.human_num, 2))
        self.soadrl_sim.last_human_states = self.update_last_human_states(
            self.soadrl_sim.human_num,
            human_visibility,
            self.soadrl_sim.humans,
            self.soadrl_sim.last_human_states,
            self.soadrl_sim.time_step,
            reset)
        for i in range(self.soadrl_sim.human_num):
            relative_pos = np.array([self.soadrl_sim.last_human_states[i, 0] - robot.px, self.soadrl_sim.last_human_states[i, 1] - robot.py])
    ...
    def step(self, action):
        ...
        if not self.LEGACY_MODE:
            ...
            # print("robot.px", "%.2f" % self.soadrl_sim.robot.px)
            # print("robot.py", "%.2f" % self.soadrl_sim.robot.py)
            # print("robot.vx", "%.2f" % self.soadrl_sim.robot.vx)
            # print("robot.vy", "%.2f" % self.soadrl_sim.robot.vy)
            # print("robot.theta", "%.2f" % self.soadrl_sim.robot.theta)
            # print("pedestrian", self.soadrl_sim.humans[0].px, self.soadrl_sim.humans[0].py)
            # os.system('cls||clear')
            dr = action[1] * self._get_dt()
            ...
            soadrl_action = ActionRot(vx_baselink, dr)
            # step
        ...
        if done:
            if self.collect_statistics:
                self.episode_statistics.loc[len(self.episode_statistics)] = [
                    ...
                    100 if isinstance(info['event'], Collision) or isinstance(info['event'], CollisionOtherAgent) or isinstance(info['event'], Timeout) else 0,
                    self.steps_since_reset,
                    isinstance(info['event'], ReachGoal),
                    reward,
                    self.soadrl_sim.human_num,
    ...
    def reset(self):
        ...
        random_rot = ActionRot(0, 10.*(np.random.random()-0.5))
        ...
        obs = self._convert_obs(reset=True)
        if self.LEGACY_MODE:
            state, local_map, reward, done, info = self.soadrl_sim.step(
                ActionRot(0, 0), compute_local_map=True, border=self.border)
            obs = (state, local_map)
        return obs
...
#                     self.transform.set_scale(20, 20)
                ...
                if RENDER_LIDAR:
                    ...
                    is_in_fov = np.cos(lidar_angles - angle) >= np.cos(self.soadrl_sim.robot_fov/2)
