...
from a.crowd_sim.envs.utils.robot import Robot
from a.crowd_sim.envs.utils.info import *
from a.crowd_sim.envs.utils.utils import point_to_segment_dist
from a.crowd_sim.envs.utils.state import ObservableState
from a.crowd_nav.policy.policy_factory import policy_factory
from a.crowd_sim.envs.utils.shape import AgentHeadingRect

Obstacle = collections.namedtuple(
    "Obstacle", ["location_x", "location_y", "dim", "patch"])


class CrowdSim2(gym.Env):
    """!
    ...
    def configure(self, config, silent=False):
        ...
        print("AAAAAAAAAAAAAAAAAAAAAAAAA", self.robot_fov)
        self.human_fov = np.pi * config.getfloat('humans', 'FOV')
        # print("BBBBBBBBBBBBBBBBBB", config.getfloat('humans', 'FOV'))
        self.last_human_states = np.zeros((self.human_num, 5))
        self.random_goal_changing = config.getboolean('humans', 'random_goal_changing')
        if self.random_goal_changing:
            self.goal_change_chance = config.getfloat('humans', 'goal_change_chance')

        # configure randomized goal changing of humans after reaching their respective goals
        self.end_goal_changing = config.getboolean('humans', 'end_goal_changing')
        if self.end_goal_changing:
            self.end_goal_change_chance = config.getfloat('humans', 'end_goal_change_chance')
        
        self.last_acceleration = (0,0)
        self.min_personal_space = 0.2


    def get_num_human_in_fov(self, human_num, robot, humans, robot_fov, human_fov):
        human_ids = []
        #print("humans.size()", len(humans), "human_num", human_num)
        humans_in_view = []
        num_humans_in_view = 0
        #print("AAAAAAAAAAA", human_num)
        for i in range(len(humans)):
            visible = self.detect_visible(robot, humans[i], robot_fov, human_fov, robot1=True)
            if visible:
                humans_in_view.append(humans[i])
    ...
    def detect_visible(self, state1, state2, robot_fov, human_fov, robot1 = False, custom_fov=None):
    ...
    def update_last_human_states(self, human_num,  human_visibility, humans, last_human_states, time_step,  reset):
        """
        ...
        ind = min(len(humans), human_num)
        for i in range(ind):
            #print(i)
            if human_visibility[i]:
                humanS = np.array([humans[i].px, humans[i].py, humans[i].vx, humans[i].vy, humans[i].radius])
                last_human_states[i, :] = humanS
    ...
    def reset(self, phase='test', test_case=None, imitation_learning=True, compute_local_map=True):
        """!
    ...
    def step(self, action, update=True, compute_local_map=True, border=None):
        ...
        # EVALUATION FROM RELATIONALGRAPHLEARNING

        ax = action.v * np.cos(action.r) - self.robot.vx
        ay = action.v * np.sin(action.r) - self.robot.vy
        d_ax = ax - self.last_acceleration[0]
        d_ay = ay - self.last_acceleration[1]
        jerk_cost = d_ax**2 + d_ay**2
        # print("action", action.v, action.r)
        # print("robot", self.robot.vx, self.robot.vy)
        self.last_acceleration = (ax, ay)
        self.vx_arr.append(self.robot.vx)
        self.vy_arr.append(self.robot.vy)
        self.ax_arr.append(ax)
        self.ay_arr.append(ay)
        self.jerk_arr.append(jerk_cost)

        side_preference = None
        h = self.humans[0]
        if ((self.humans[0].py - self.humans[0].radius) <= end_position[1] <= (self.humans[0].py + self.humans[0].radius)):
            if end_position[0] < self.humans[0].px:
                side_preference = 0
            else:
                side_preference = 1
        
        heading_rect_violations = 0
        robot_rect = AgentHeadingRect(self.robot.px, self.robot.py, self.robot.radius, self.robot.vx, self.robot.vy, action)
        for human in self.humans:
            human_rect = AgentHeadingRect(human.px, human.py, human.radius, human.vx, human.vy)
        ...
        # print(info['social_violation_cnt'])
        info['jerk_cost'] = jerk_cost
        info['speed'] = action.v
        info['side_preference'] = side_preference
        info['collision_pedestrian'] = False
        ...
        if self.global_time >= self.time_limit:
            ...
            info['collision_pedestrian'] = collision_pedestrian
            collision_pedestrian = False
        ...
        #     info = Nothing()
            ...
            # print('potential_cur', potential_cur)
